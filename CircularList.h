...
DoubleNode<T>* CircularList<T>::find(int index) 
{
   ...
   if (index >= loc_pos)
   {
        dist_next = index + sze - loc_pos;  //distance without the bridge (next refs, positive)
        dist_prev = loc_pos - index;          //distance using the bridge (prev refs, negative)
   }
   else
   {
          dist_prev = index - (loc_pos+sze);                          //distance without the bridge (prev refs, negative)
